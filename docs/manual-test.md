...
- 作業ディレクトリ: `D:\AI\WindowsApp\subtitle-telop-multilingual-width-check`
...
```powershell
cd D:\AI\WindowsApp\subtitle-telop-multilingual-width-check
npm test
npm start
```
...
5. Windows-first CLI + local preview UI の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/subtitle-telop-multilingual-width-check-docs.zip` に存在する。
